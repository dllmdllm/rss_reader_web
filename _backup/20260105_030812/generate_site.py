        ...
        for img in root.xpath(".//img"):
            ...
            if src:
                ...
                if "cnbeta.com.tw" in base_url:
                    ...
                    if "cnbeta.com.tw" in parsed.netloc and "/images/" in parsed.path:
                        img.drop_tag()
                        continue
...
def parse_items(payload: bytes | str, source: str) -> list[Item]:
    ...
    try:
        ...
        for item in root.findall(".//item"):
            ...
            items.append(
                Item(
                    title=to_trad(strip_html(title)),
                    link=link,
                    pub_dt=pub_dt,
                    pub_text=pub_text,
                    source=source,
                    summary=to_trad(strip_html(summary)),
                    rss_image=rss_image,
                )
            )
        return items
...
def build_html(
    ...
    for idx, item in enumerate(items, start=1):
        ...
        content = clean_content_text(to_trad(strip_html(content)))
        ...
        if image_url:
            ...
            if local_name:
                ...
                if item.source == "cnbeta":
                    hero_caption = f"<div class='img-note'>圖檔：{html.escape(local_name)}</div>"
            else:
                hero_url = f"{image_url}?v={build_id}"
                if item.source == "cnbeta":
                    hero_caption = f"<div class='img-note'>圖源：{html.escape(image_url)}</div>"
            if item.source != "cnbeta":
